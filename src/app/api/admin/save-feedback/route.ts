...
async function sendEmail(to: string, subject: string, html: string) {
  ...
  try {
    ...
    console.error("[save-feedback] Email error:", e);
    return false;
  }
}
    ...
    <div style="font-family:Georgia,serif;max-width:600px;margin:0 auto;padding:32px 24px;background:#f8f7f4;">
      ...
      <div style="background:#ffffff;padding:32px;border-radius:0 0 12px 12px;border:1px solid rgba(15,76,92,0.15);border-top:none;">
        ...
        <div style="text-align:center;margin:28px 0 20px 0;">
          <a href="https://mrcpiobgynunlocked.com/dashboard" style="background:#15B097;color:#0B1E3D;padding:14px 32px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px;display:inline-block;">View in My Dashboard →</a>
        </div>
      ...
      <p style="text-align:center;font-size:12px;color:rgba(26,26,26,0.35);margin-top:16px;">© ${new Date().getFullYear()} MRCPI OBGYN Unlocked &nbsp;·&nbsp; <a href="https://mrcpiobgynunlocked.com" style="color:rgba(26,26,26,0.35);">mrcpiobgynunlocked.com</a></p>
    </div>
  `;
}
...
export async function POST(req: Request) {
  ...
  const { feedbackId, studentId, studentEmail, studentName, title, content, feedbackType, isUpdate } = await req.json();

  const serviceClient = createServiceClient();

  if (isUpdate && feedbackId) {
    const { error } = await serviceClient.from("student_feedback").update({
    ...
    const { error } = await serviceClient.from("student_feedback").insert([{
  ...
  const subject = isUpdate ? `Your feedback has been updated: ${title}` : `New feedback from your instructor: ${title}`;
